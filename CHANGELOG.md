...
- **Healthy Live/Postgres status dots are now static across every view.** Their
  ...
  warning and activity indicators retain a compositor-safe opacity pulse with
  no animated shadow repaint, including every Dream Ready indicator.
...
- **`evals/prompts/sonnet_extractor_v5.md` is the new default for the
  ...
  manual-start hints in `ops/install.{ps1,sh}`). It is v4 with the v2
  body's two pre-rule worked examples re-cut on invented names — the same
  re-cut the daemon's shipped prompt took with the v12 base earlier the
  same day (`road bike` → `penny-farthing`; `Northern Flicker` / `32` /
  `at the park` → `Gallowmere Teal` / `41` / `Kelmarsh Reserve`) — and
  nothing else: same rules, same assistant-facts blocks, same JSON shape.
  The daemon-side re-cut alone could not reach an install whose primary
  extractor is the shim, because `--system-prompt-file` replaces the
  shipped prefix; this closes the last deployed path that named
  LongMemEval `affe2881`'s answer. Generated by `evals/gen_shim_prompt.py`
  from `sonnet_extractor_v2.md` through `V5_RECUTS`; v2 and v4 are not
  edited (each is the pre arm of a committed gate), and
  `tests/test_shim_prompt.py` holds v5 to exactly the registered pairs, an
  EMPTY corpus-collision list, and a dataset grep over every name it
  carries.
- **Gated on the ladder `opus-5` rung before the default moved** (the
  Max-plan CLI shim on its dedicated port 8083; the live `:8082` shim was
  never addressed), v4 vs v5, two replicates per arm:
  `evals/results/ladder-shimv5-paired-verdict-threshold.json` reads
  `gate: PASS` and `no_regression_gate: PASS`. Gold 1.0, stale 0.0 and an
  identical tally (26 pulled, 16 claims, 16 inserted, 0 superseded) on all
  four runs; tokens/query 14.3 and 14.5 against the pre arm's 15.7 and
  14.1 — the post arm sits inside the pre arm's own spread, so nothing is
  claimed for the move. Said plainly in `evals/README.md`: this ladder
  saturates on prompt changes and is the right gate here only because the
  diff is six token substitutions and no rule change. Pinned in
  `tests/test_eval_evidence.py`, launcher comments included.
- An existing install picks v5 up when the shim autostart is re-installed
  or the shim is restarted with the new file; the daemon image is not
  involved.
...
- **Three worked examples in the extraction-prompt lineage were written from
  ...
  count-exclusion example in the shipped `_SYSTEM_PROMPT` ("saw a Northern
  Flicker today, that makes 32 species at the park now" → value `32`)
  paraphrases question `affe2881`'s own answer turn, and `32` is its gold;
  `evals/results/c2op-count-census.json` lists that question among the seven
  `frozen-total` losses the rule was written to recover, and the gate that
  shipped the rule (`c2op-count-verdict.json`) ran on the 78-question slice
  containing it. The collection-membership example's value `road bike` is the
  gold of `89941a94` and `gpt4_e414231f`, and the unshipped
  `events_pass_v2.txt` jam example is question `2b8f3739`'s answer turn with
  one word removed. All three entered on 2026-08-01 (`a4686df6`; the op block
  a day earlier) — every run before that date is untouched, including the
  Sonnet v1 project record — and leave-one-out over every affected doc-cited
  artifact moves no extractor arm by more than 0.0044 (ceiling-v38 cortex,
  0.6667 → 0.6623; `evals/results/prompt-example-lift-audit-20260905.json`,
  regenerated by `evals/prompt_lift_audit.py`). `affe2881` is cortex-correct
  in 82 of 90 pre-change oracle-KU artifacts (replicates included) and
  wrong in all 58 `_s` artifacts of either era, so the example is not
  load-bearing. The defect is the method — a rule fitted to a diagnosed
  benchmark item and gated on the slice holding it — not the numbers.
- `tests/test_prompt_example_lifts.py` guards the whole lineage (the live
  `dream.py` constants and every `evals/prompts/*` file) with two predicates,
  each blind to what the other catches: worked-example values against every
  gold answer (finds `32` and `road bike`, cannot see the jam example, whose
  $225 is a summand of the $495 gold), and word 4-gram shingles of every
  example note against both dataset files (finds the Flicker and jam
  sentences, cannot see `road bike`). Both are equalities against a dated
  allowlist keyed by (carrier, example): a new lift fails, and an entry that
  stops hitting must be deleted. The debt is recorded, not paid — the shipped
  prompt is byte-pinned to its measured artifact and re-cutting it is a
  prompt change with its own ladder gate; the archived `ku_op_prompt_v*`
  variants are the arms of past gates and stay as they were measured.
- Three carriers landed between the audit and this guard and inherit both
  examples unchanged, so they are listed as debt with the same 2026-09-05
  rationale: `assistant_facts_naive.txt` and `assistant_facts_provenance.txt`
  (the v10 base plus the assistant-facts blocks; the provenance file is the
  shipped prompt since 2026-09-05) and `sonnet_extractor_v4.md` (v2 plus the
  same blocks, the deployed shim default since 2026-09-06). The scan reads a
  note that opens with a bare `assistant:` marker the way it reads a
  `[date] user:` stamp, and the stop-list self-check lets an article open a
  Titlecase name ("The Quillon Larder") — the name's own tokens stay in the
  scan, so nothing is hidden by dropping its "the".
